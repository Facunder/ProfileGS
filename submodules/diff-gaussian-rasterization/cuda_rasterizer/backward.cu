...
)
{
	...
	const half2 pixf = { (__half)pix.x, (__half)pix.y };
	...
	__shared__ __half collected_depths[BLOCK_SIZE];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const half2 xy = collected_xy[j];
			const half2 d = { xy.x - pixf.x, xy.y - pixf.y };
			...
			const __half G = __half(exp(float(power)));
			const __half alpha = __hmin(__half(0.99f), con_o.w * G);
			if (alpha < __half(1.0f / 255.0f))
				continue;

			T = T / float(__half(1.0) - alpha);
			const __half dchannel_dcolor = alpha * __half(T);
			...
			for (int ch = 0; ch < C; ch++)
			{
				...
				accum_rec[ch] = last_alpha * last_color[ch] + (__half(1.f) - last_alpha) * accum_rec[ch];
				last_color[ch] = c;
			...
			if (dL_dinvdepths)
			{
			const __half invd = __half(1.f) / collected_depths[j];
			accum_invdepth_rec = last_alpha * last_invdepth + (__half(1.f) - last_alpha) * accum_invdepth_rec;
			last_invdepth = invd;
			...
			for (int i = 0; i < C; i++)
				bg_dot_dpixel += __half(bg_color[i]) * dL_dpixel[i];
			dL_dalpha += __half(-T_final / float(__half(1.0) - alpha)) * bg_dot_dpixel;
			...
			atomicAdd(&dL_dconic2D[global_id].x, float(__half(-0.5f) * gdx * d.x * dL_dG));
			atomicAdd(&dL_dconic2D[global_id].y, float(__half(-0.5f) * gdx * d.y * dL_dG));
			atomicAdd(&dL_dconic2D[global_id].w, float(__half(-0.5f) * gdy * d.y * dL_dG));

			// Update gradients w.r.t. opacity of the Gaussian
			atomicAdd(&(dL_dopacity[global_id]), float(G * dL_dalpha));
		}
	}
}
